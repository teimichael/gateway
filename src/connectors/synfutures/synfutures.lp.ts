...
import { SynFuturesLPHelper } from './synfutures.lp.helper';
import { AddPosReturn } from './synfutures.lp.interfaces';
...
export class SynFuturesLP extends SynFuturesLPHelper implements UniswapLPish {
  private static _instances: { [name: string]: SynFuturesLP };
  private _gasLimitEstimate: number;

  private constructor(chain: string, network: string) {
    super(chain, network);
    this._gasLimitEstimate = SynFuturesConfig.config.gasLimitEstimate;
  }

  public static getInstance(chain: string, network: string): SynFuturesLP {
    if (SynFuturesLP._instances === undefined) {
      SynFuturesLP._instances = {};
    }
    if (!(chain + network in SynFuturesLP._instances)) {
      SynFuturesLP._instances[chain + network] = new SynFuturesLP(
        chain,
        network,
      );
    }

    return SynFuturesLP._instances[chain + network];
  }
